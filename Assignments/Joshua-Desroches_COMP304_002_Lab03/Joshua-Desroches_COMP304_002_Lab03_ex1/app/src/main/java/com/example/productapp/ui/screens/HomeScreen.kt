...
fun HomeScreen(
    ...
    val filteredProducts by remember(searchName, products) {
        derivedStateOf {
            ...
            if (q.isBlank()) products
                ...
                products.filter { product ->
                    product.name.contains(q, ignoreCase = true) || product.id.toString().contains(q)
                }
            }
        }
    }

    // If the selected product is no longer in the filtered results, clear the selection
    LaunchedEffect(searchName, products) {
    ...
    Scaffold(
        ...
        if (isExpandedScreen) {
            ...
            LazyColumn(modifier = Modifier.padding(paddingValues)) {
                // use filteredProducts here as well
                items(filteredProducts) { product ->
                    ProductItem(
                        product = product,
